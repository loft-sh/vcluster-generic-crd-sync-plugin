...
func RegisterSyncer(ctx *synccontext.RegisterContext, syncer syncer.Syncer) error {
...
func (r *syncerController) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	lifecycle, ok := r.syncer.(syncer.Starter)
	if ok {
	...
	if vObj != nil && pObj == nil {
		...
		// check if up syncer
		upSyncer, ok := r.syncer.(syncer.UpSyncer)
		if ok {
...
func (r *syncerController) Register(ctx *synccontext.RegisterContext) error {
	...
	var err error
	modifier, ok := r.syncer.(syncer.ControllerModifier)
	if ok {
